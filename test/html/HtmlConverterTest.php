...
class HtmlConverterTest extends PHPUnit_Framework_TestCase {
  public function testConvertAndRevertAtStackOverflow() {
    ...
    list($translated_html, $maker) = $converter->convertToAppropriateForApiBody();
    ...
    $actual_html_text = $maker->revert($translated_html);
    $doc = new DOMDocument( "1.0", "ISO-8859-15" );
  ...
  public function testConvertToAppropriateForApiBodyWithWovnIgnore() {
    ...
    list($translated_html, $maker) = $this->executeConvert($converter, $html, 'removeWovnIgnore');
    $keys = $maker->keys();
  ...
  public function testConvertToAppropriateForApiBodyWithForm() {
    ...
    $converter = new HtmlConverter($html, 'toK3n');
    list($translated_html, $maker) = $this->executeConvert($converter, $html, 'removeForm');
    $keys = $maker->keys();
  ...
  public function testConvertToAppropriateForApiBodyWithScript() {
    ...
    $converter = new HtmlConverter($html, 'toK3n');
    list($translated_html, $maker) = $this->executeConvert($converter, $html, 'removeScript');
    $keys = $maker->keys();
  ...
  public function testConvertToAppropriateForApiBodyWithComment() {
    ...
    $converter = new HtmlConverter($html, 'toK3n');
    list($translated_html, $maker) = $this->executeRemoveBackendWovnIgnoreComment($converter, $html);
    $keys = $maker->keys();
...
bye
  ...
  private function executeConvert($converter, $html, $name) {
    ...
    $method->invoke($converter, $dom, $maker);

    $converted_html = $dom->save();
    $dom->clear();
    unset($dom);

    return array($converted_html, $maker);
  }
  ...
  private function executeRemoveBackendWovnIgnoreComment($converter, $html) {
    ...
    $method->setAccessible(true);
    $converted_html = $method->invoke($converter, $html, $maker);
