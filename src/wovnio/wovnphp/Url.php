...
class Url
{
            ...
            if ($parsed_host !== null && strtolower($parsed_host) === strtolower($no_lang_host)) {
                switch ($pattern) {
                    case 'subdomain':
                        ...
                        if (preg_match('/\./', $parsed_url['host'])) {
                            ...
                            if ($sub_do_lang && strtolower($sub_do_lang) === strtolower($lang_code)) {
                                ...
                                $new_uri = preg_replace(
                                    '/(\/\/)([^\.]*)/',
                                    '${1}' . self::formatForRegExp(strtolower($lang_code)) . '.' . '${2}',
                                    $no_lang_uri,
                                    1
                                );
                            }
                    ...
                    default:
                        ...
                        $new_uri = preg_replace(
                            '/([^\.]*\.[^?\/]*)(\?|\/|$)/',
                            '${1}/' . self::formatForRegExp($lang_code) . '${2}',
                            $no_lang_uri,
                            1
                        );
                }
    ...
    public static function removeLangCode($uri, $pattern, $lang_code)
    {
        ...
        switch ($pattern) {
            case 'query':
                $no_lang_uri = preg_replace('/(^|\?|&)wovn=' . $lang_code . '(&|$)/i', '\1', $uri);
                return preg_replace('/(\?|&)$/', '', $no_lang_uri);
                break;
            case 'subdomain':
