...
docker cp $(pwd)  $dummy_container:/opt/
...
# Run test
docker run -t -w /opt/project --volumes-from $dummy_container $docker_name /bin/bash -c /opt/project/vendor/bin/phpunit; /opt/project/vendor/bin/phpunit --configuration phpunit_integration.xml
