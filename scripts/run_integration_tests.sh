...
if [[ "${DOCKER_IMAGE}" =~ ^.*php:?5\.3.*$ ]]; then
    ...
    docker exec ${APACHE_CONTAINER_ID} /bin/bash -c "cd ${WORK_DIR}; cp ./scripts/isrg-root-x1-cross-signed-new.crt /usr/local/share/ca-certificates/isrg-root-x1-cross-signed-new.crt"
